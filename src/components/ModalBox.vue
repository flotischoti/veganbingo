<template>
  <div class="modalBox" :class="{fadeIn: show, fadeOut: !show}">
    ...
    <div class="controls">
      <button class="close" @click="$emit('close')"><i class="fas fa-times"></i> {{$t("message.board.popup.close")}}</button>
      <button class="copy" @click="copy"><i class="fas fa-copy"></i>  {{$t("message.board.popup.copy")}}</button>
...
export default defineComponent({
  ...
  computed: {
    title() {
      return this.field && this.field.title ? this.$t(`${this.field.title}`).replace(/(<([^>]+)>)/ig, "") : '';
    }
  },
  methods: {
    copy() {
      navigator.clipboard.writeText(this.stripText(this.$t(this.field?.title)) + ' - ' +  this.stripText(this.$t(this.field?.text)));
      this.$emit('copy');
    },
...
.modalBox {
  ...
  &.fadeIn {
      ...
      height: 98.5%;
      z-index: 200;
      transition: max-width .5s 0.3s, height .5s .8s;
    }

  &.fadeOut {
    transition: max-width .5s 0.502s, height .5s, z-index 0s 1.05s;
  }
