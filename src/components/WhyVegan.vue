  ...
  <div class="WhyVeganContainer">
    <article v-for="(slider, index) in veganArguments" :key="index">
      <div class="textWrapper">
        ...
        <p
          v-for="(text, index) in slider.texts"
          :key="index"
          v-html="$t(`${text.text}`)"
      ...
      <div
        ...
        :style="{
          'background-color':
            'hsla(' + 250 * (57 + slider.id) + ', 80%, 25%, .8)',
        }"
      >
        <ul
          class="controls"
          :id="slider.controlsId"
        ...
        <div :class="slider.sliderClass">
          <div
            ...
            :key="index"
            class="slider-item"
            :style="{ 'background-color': 'hsl(0, 80%, 25%, 0)' }"
          >
            <div
              class="card"
              :style="{
                'background-color':
                  'hsl(' + 250 * (25 + slider.id) + ', 80%, 25%, 0)',
              }"
            >
              <h2 v-if="item.customHeader">
                {{ item.customHeader }} {{ index + 1 }}
              </h2>
              <i v-if="!item.customHeader" :class="item.classes">
                {{ ' ' + (index + 1) }}</i
              >
